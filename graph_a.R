...
col = c("yellow","lightsalmon", "lavenderblush","darkolivegreen2" , "cyan3", "grey") #Colors of the nodes se http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf
...
bind_rows(transmute(df_edges,name=source,label=source_label),
  ...
  mutate(label=ifelse(rbinom(n(),1,1),actual_label,""),color=colorise(name))->df_nodes #change the number of labels to be labeled here
...
plot(g_frame,
     ...
     vertex.label.cex = 0.3,#Change label size here
...
legend("bottomleft", 
       legend = c("Shelfmarks" , "http://...manuscript/view",  "bibliography" , "biography", "place", "Other"), 
       col = col, 
       pch = c(20,20,20,20,20), bty = "y",  pt.cex =2.5, cex = 0.4 , #pt.cex = bubble size
